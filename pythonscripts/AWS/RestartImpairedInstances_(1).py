...
def stop_instances(impaired_instances, ec2_client, wait):
...
def lambda_handler(event, context):
    ...
    autoscaling_impaired_instances = []
        ...
        if f2["Reservations"] :
            for r in f2["Reservations"]:
                for i in r["Instances"]:
                    ...
                    autoscaling_impaired_instances.append(i["InstanceId"])
    ...
    if impaired_instances:
        ...
        stop_instances(non_autoscaling_impaired_instances, ec2_client, True)
        stop_instances(autoscaling_impaired_instances, ec2_client, False)
